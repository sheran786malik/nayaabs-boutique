import { Dimensions, TouchableOpacity, Image, TextInput, SafeAreaView, FlatList, View, Text } from 'react-native'
import React, { useState, useEffect } from 'react'
import { useNavigation } from '@react-navigation/native';
import { AntDesign } from 'react-native-vector-icons'
import { AdjustmentsIcon, ChevronLeftIcon, SearchIcon, PlusIcon, MinusIcon, ThumbDownIcon, ArrowCircleRight, } from 'react-native-heroicons/outline';
      ...
      <Card>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Product', {
          ...
          <View className='flex-row w-32 justify-between bg-dark'>
            <Text claßssName='w-20' style={{ color: 'black' }}>{item.name}</Text>
    ...
    return (
      <View className='p-5 bg-white m-2 rounded-3xl' key={item.id} style={{ width: Dimensions.get('screen').width / 2.5 }}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Product', {
          ...
          <Image source={item.image} style={{ width: 100, height: 100, margin: 8 }} />
